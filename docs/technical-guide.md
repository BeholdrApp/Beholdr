# Beholdr technical guide

Deployment, configuration, API, and development reference. For the project goal and a quick start, see the [README](../README.md).

## Stack

- **Backend** — Go 1.27.1, standard-library `net/http` (method+pattern routing),
...
- **Service health** — bounded Prometheus-backed charts from one hour through
  21 days for HTTP 5xx rate (with the week-before series),
  CPU usage against limits (or requests, if configured),
...
Open http://localhost:5173 for the UI. The backend at :8000 serves the API and a placeholder page unless the built UI has been copied into internal/webui/dist before compiling Go.
...
Compose mounts ${HOME}/.kube at /kube. Set HOME to your user directory if needed (PowerShell: `$env:HOME = $env:USERPROFILE`). The API server must be reachable from the container, certificate paths must resolve inside it, and kubeconfigs using external credential plugins require those tools, which the minimal image does not include. Use the two-process setup above when your kubeconfig depends on host tooling.
...
cd deploy/terraform
cp terraform.tfvars.example terraform.tfvars   # set image, ingress host, TLS, and auth
terraform init
terraform apply
```
...
See [terraform.tfvars.example](../deploy/terraform/terraform.tfvars.example) and [Security model](#security-model) above.
Prefer raw manifests? See [beholdr.yaml](../deploy/k8s/beholdr.yaml).
